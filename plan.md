...
Borrowed from [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything). Layers visualization and exploration on the approved local Tier-3 graph/vector stores. `stratum/scripts/understand-codebase.ts` provides a read-only entity/semantic-query CLI with a universal bound slash adapter. Project-local JS/TS/Rust File and Function ingestion, fuzzy and semantic node search, paged dependency tours, exact-path durable Tier-2 links, selectable in-canvas fact nodes, and a real-Chrome dashboard check are implemented; Python extraction and model summaries remain open.

- [ ] **Knowledge-graph extraction pipeline**. Project-local JS/TS/Rust ingestion walks files, extracts top-level functions and local imports/modules, and writes File/Function nodes with DECLARES/DEPENDS_ON edges. A dedicated edge table durably links indexed Files to active, exact-path Tier-2 changes and decisions; the canvas shows those links when a File is selected. Python declarations and model-generated source summaries remain open.
