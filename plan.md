...
- [ ] **Dashboard `/dashboard/graph` view** (~10h). A vanilla JS route now offers pan, zoom, click-to-expand, and a sidebar with node paths, source-derived summaries, and relationships from the bounded API. Browser verification, pagination, generated plain-English summaries, and related Tier-2 facts remain open.
