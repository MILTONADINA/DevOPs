...
Borrowed from [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything). Layers visualization and exploration on the approved Supabase Tier-3 graph/vector stores. `stratum/scripts/understand-codebase.ts` provides a read-only entity/semantic-query CLI with a universal bound slash adapter. Project-local JS/TS File and Function ingestion, fuzzy search, paged dependency tours, exact-path durable Tier-2 links, and a real-Chrome dashboard check are implemented; cross-language extraction, model summaries, semantic node search, and in-canvas fact nodes remain open.

- [ ] **Knowledge-graph extraction pipeline**. Project-local JS/TS ingestion walks files, extracts top-level functions and relative imports, and writes File/Function nodes with DECLARES/DEPENDS_ON edges. A dedicated edge table durably links indexed Files to active, exact-path Tier-2 changes and decisions. Cross-language declarations and in-canvas fact nodes remain open.
- [x] **`/understand-codebase` slash command**. The universal command invokes the read-only CLI through a trusted project/org adapter. Graph dashboard linking awaits the dashboard.
- [ ] **Dashboard `/dashboard/graph` view** (~10h). A vanilla JS route offers pan, zoom, click-to-expand, fuzzy search, and a paged File/dependency tour. The sidebar shows node paths, source-derived summaries, relationships, and active Tier-2 facts through durable exact-path links. A local Chrome check verifies load, selection, literal fact rendering, search, and tour navigation. Generated plain-English summaries and in-canvas fact nodes remain open.
- [ ] **Fuzzy + semantic search across the graph** (~4h). The dashboard now searches node names across the full organization graph, including nodes outside its 500-node snapshot, with fuzzy matching and immediate neighbors. The CLI supports semantic fact search via Supabase pgvector; semantic node search and dashboard integration remain open.
- [ ] **Auto-generated guided tours** (~3h). The dashboard now traverses all File/dependency pages and generates deterministic dependency-first order with cycle handling, Previous/Next controls, paths, and source summaries. A local Chrome run verifies dependency-first Next navigation. Generated plain-English narration remains open.
