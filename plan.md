...
Borrowed from [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything). Layers visualization and exploration on the approved Supabase Tier-3 graph/vector stores. `stratum/scripts/understand-codebase.ts` provides a read-only entity/semantic-query CLI with a universal bound slash adapter. Project-local JS/TS File and Function ingestion, fuzzy search, paged dependency tours, and exact-path active Tier-2 fact reads are implemented; cross-language extraction, persistent Tier-2 graph links, model summaries, semantic node search, and real-browser verification remain open.

- [ ] **Knowledge-graph extraction pipeline**. Project-local JS/TS ingestion now walks files, extracts top-level functions and relative imports, and writes File/Function nodes with DECLARES/DEPENDS_ON edges. Cross-language declarations and links to Tier-2 decisions and deprecations remain open.
