...
Borrowed from [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything). Layers visualization and exploration on the approved local Tier-3 graph/vector stores. `stratum/scripts/understand-codebase.ts` provides a read-only entity/semantic-query CLI with a universal bound slash adapter. Project-local JS/TS File and Function ingestion, fuzzy and semantic node search, paged dependency tours, exact-path durable Tier-2 links, selectable in-canvas fact nodes, and a real-Chrome dashboard check are implemented; cross-language extraction and model summaries remain open.
