-- Classify public.invoice_send_claims (20260924235900) in the session erasure
-- inventory. The table is org-scoped (org_id, billing period; no session_id),
-- like public.invoices, so it is an org-only class: session erasure neither
-- counts nor deletes it. Without this, the inventory's all-public-tables
-- coverage guard (test/integration/local-session-erasure-inventory.sql) fails.
-- The function body is otherwise identical to 20260924235500.
...
SELECT jsonb_build_object(
  ...
  'org_only_classes', jsonb_build_array('invoices', 'invoice_send_claims', 'api_keys', 'developers', 'org_config', 'organizations'),
