...
| v0.4.x | Phase 2 KadaneDial pruner + subagent-driven-development autonomous loops | IN PROGRESS (Session 16: pruner core + real ONNX encoder + real LLM judge + eval gate engine + Tier-B suite 5/5 BUILT, validated & adversarially-verified across 2 review cycles. Remaining: published Tier-A eval [gated on datasets] + request-path wiring + autonomous loops) | ~30 | ~105 | ~22% | Eval GREEN; zero Tier C regressions; 1 wk no degradation; autonomous loop pilot succeeds |
