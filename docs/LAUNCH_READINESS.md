...
- **For "is the full vision done?"** → **~36%** (Option B locked; v0.3.x nearly done, v0.4.x pruner built). ~290h done / ~799h envelope = ~36.3% → headline **~36%**. Session 16 added ~122h by crediting completed+verified scope at this doc's per-scope estimates (see Math caveat — not newly-tracked hours). The remaining ~509h to v1.0.0 is dominated by EXTERNAL/USER gates: Tier-A datasets (ship pruning), Pinecone+Neo4j (v0.5.x), AWS Nitro TEE (v0.7.x), Stripe + design partner (v1.0.0), PB-16 signing key, PB-13 Actions billing.
- **For "what's the next load-bearing decision?"** → it is now USER-side: supply the **Tier-A eval datasets** (LoCoMo/MT-Bench+/SCM4LLMs) so the published accuracy gate can run and pruning can move from shadow-mode into the request path (the v0.4.x ship gate). The eval harness + real judge + real encoder are all built and dev/Tier-B-validated. Parallel user-side gates unchanged: PB-13 (Actions billing + `CLAUDE_API_KEY` repo secret for the security-review workflow), PB-16 (signing key for the v0.3.0 tag).
