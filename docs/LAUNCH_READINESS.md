...
| v0.5.x | Phase 3 three-tier memory + knowledge-graph view | IN PROGRESS (Session 17: **Tier-2 warm-memory adapter + session/org resolver BUILT**, warm-write path composes end-to-end [evict→extract→resolve FKs→persist→queryRecent] + live Supabase `initial_schema` applied to the Stratum project + **live SQL round-trip verified** + 22 unit/integration tests + ADR-0012 + gated `verify-tier2` script. Tier-1 hot + Zod fact schemas + extractor built/tested in prior sessions. Tier-3 Neo4j + Pinecone still STUBS [gated on accounts]; knowledge-graph view + `/understand-codebase` not started) | ~32 | ~98 | ~25% | Fact-survives-50-turn; tier latencies met; `/understand-codebase` works |
...
| **TOTAL to v1.0.0** | — | — | **~322** | **~477** | **~40.3%** | — |

**Session 17 delta (v0.5.x Tier-2 warm memory): 36% → ~40%.** +~30h credited to v0.5.x,
split honestly: **~15h is THIS session's fully-verified work** (Tier-2 persistence
adapter built; the repo's `initial_schema` migration applied to the *live* Stratum
Supabase project; a live insert→select→cleanup round-trip proven via the exact row
shapes the adapter emits; 15 credential-free unit tests; ADR-0012; a gated
`verify-tier2` live-integration script), and **~15h corrects a stale undercount** —
the v0.5.x *core* (Tier-1 hot memory + Zod fact schemas + the security-hardened fact
extractor) was built and unit-tested in prior sessions but left at 0% in the table.
...
- **For "is the full vision done?"** → **~40%** (Option B locked; v0.3.x nearly done, v0.4.x pruner built, v0.5.x Tier-2 warm memory now live + built). ~320h done / ~799h envelope = ~40.1% → headline **~40%**. Session 17 added ~30h (split: ~15h verified Tier-2 this session + ~15h correcting a stale v0.5.x-core undercount — see Math). The remaining ~479h to v1.0.0 is dominated by EXTERNAL/USER gates: Tier-A datasets (ship pruning), **Neo4j + Pinecone** (the rest of v0.5.x — Tier-3 cold), AWS Nitro TEE (v0.7.x), Stripe + design partner (v1.0.0), PB-16 signing key, PB-13 Actions billing. **One prior gate is now satisfied**: the Supabase account (Tier-2 store) — used this session.
- **For "is v0.5.x Tier-2 warm memory done?"** → **BUILT + live-schema-verified, off-path.** The repo `initial_schema` migration is applied to the live Stratum Supabase project (13 tables); the warm-memory adapter (`src/memory/warm/tier2.ts`) routes each fact type to its table with trusted-FK injection (forgery-safe) + FAIL-CLOSED validate-before-write + `queryRecent`; a live insert→select→cleanup round-trip is proven (via MCP `execute_sql`, exact adapter row shapes) and the pure logic is covered by 15 credential-free unit tests (full suite 282 pass + 5 todo). ADR-0012 anchors the design. The live **TS-client** round-trip (`npm run verify-tier2`) is gated on `SUPABASE_SERVICE_KEY` in `.env` (skips cleanly without it). Tier-3 (Neo4j/Pinecone) still stubbed + gated on accounts.
