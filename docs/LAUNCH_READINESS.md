...
| v0.3.x | Phase 0 complete + Phase 1 measurement proxy + first-party Anthropic integrations | IN PROGRESS (§2a + §2f CLOSED Session 15; §2b/§2c/§2d/§2g/§2h pending) | ~11 | ~104 | ~10% | Friend can clone + setup + see live dashboard; `/security-review` GitHub Action gates PRs |
...
| **TOTAL to v1.0.0** | — | — | **166** | **~633** | **~20.8%** | — |

Math: 166h done ÷ ~799h total envelope = **~20.8% to v1.0.0** (rounds to headline **~21%**). Session 15 added ~11h to numerator (8h v0.3.x §2a code-side gaps closure + 3h v0.3.x §2f Anthropic first-party integrations closure); envelope unchanged.
...
- **For "is the full vision done?"** → **~21%** (Option B locked, v0.3.x in progress). 155h (v0.2.0 SHIPPED) + 11h (Session 15 v0.3.x §2a + redactor bug fix + §2f Anthropic integrations + in-session validator-rot remediation) = **166h done / ~799h total envelope = 20.8% → headline ~21%**. First numerator-loaded implementation work since v0.2.0 sealing. v0.3.x ~104h remaining = §2b content corpus (~12h) + §2c Phase 0 closure (~few hours) + §2d Phase 1 measurement proxy (~80h) + §2g/§2h closure (~few hours) + release (~4h).
...
Both refactors follow the PB-19/20/22 family pattern (Session 13). Both reproducibility hashes still match (test_command paths unchanged). Both check scripts now resilient to future legitimate LR heading evolution. **Validator final state: 95/96 valid** (was 94/95 pre-Session-15; net delta = +2 emitted claims + 2 rot defects fixed = full corpus stronger). The 1 fail remains claim 076 (PB-21 PB-13-coupled exception, documented and expected).
