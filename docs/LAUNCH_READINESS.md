...
| v0.5.x | Phase 3 three-tier memory + knowledge-graph view | IN PROGRESS (Session 17: **Tier-2 warm-memory adapter + session/org resolver + MemoryManager BUILT**; **ship-gate criterion "fact-survives-50-turn" MET** (logic-proven: a decision aged out of hot memory is extracted→persisted→recalled from warm 50 turns later); warm-write path composes end-to-end + live Supabase `initial_schema` applied + **live SQL round-trip verified** + 24 unit/integration tests + ADR-0012 + gated `verify-tier2`. Tier-1 hot + Zod schemas + extractor built prior. Remaining: Tier-3 Neo4j + Pinecone STUBS [gated on accounts]; "tier latencies met" + knowledge-graph view/`/understand-codebase` not started) | ~37 | ~93 | ~28% | ✅ Fact-survives-50-turn (logic); ⬜ tier latencies; ⬜ `/understand-codebase` |
