...
- **For "is the full vision done?"** → **~42%** (Option B locked; v0.3.x nearly done, v0.4.x pruner built, v0.5.x warm memory live + built + one ship-gate criterion met + Tier-3 graph built on Supabase). ~338h done / ~799h envelope = ~42.3% → headline **~42%**. Session 17 added ~45h (split: ~30h verified this session [Tier-2 adapter + resolver + MemoryManager + fact-survives-50-turn + pruning_logs + **Tier-3 knowledge graph**] + ~15h correcting a stale v0.5.x-core undercount — see Math). The remaining ~461h to v1.0.0 is dominated by EXTERNAL/USER gates: Tier-A datasets (ship pruning), AWS Nitro TEE (v0.7.x), Stripe + design partner (v1.0.0), PB-16 signing key, PB-13 Actions billing. **Gates now satisfied**: the Supabase account (Tier-2 + Tier-3 store) — used this session; **Neo4j/Pinecone are no longer hard blockers** for v0.5.x (Tier-3 built on Supabase per ADR-0013, swappable later).
