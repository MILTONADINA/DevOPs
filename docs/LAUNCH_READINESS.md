...
- **For "is the full vision done?"** → **~41%** (Option B locked; v0.3.x nearly done, v0.4.x pruner built, v0.5.x warm memory now live + built + one ship-gate criterion met). ~327h done / ~799h envelope = ~40.9% → headline **~41%**. Session 17 added ~37h (split: ~22h verified this session [Tier-2 adapter + resolver + MemoryManager + fact-survives-50-turn] + ~15h correcting a stale v0.5.x-core undercount — see Math). The remaining ~472h to v1.0.0 is dominated by EXTERNAL/USER gates: Tier-A datasets (ship pruning), **Neo4j + Pinecone** (the rest of v0.5.x — Tier-3 cold), AWS Nitro TEE (v0.7.x), Stripe + design partner (v1.0.0), PB-16 signing key, PB-13 Actions billing. **One prior gate is now satisfied**: the Supabase account (Tier-2 store) — used this session.
...
- **Tested.** `test/memory/{tier2,sessions,warm-pipeline}.test.ts` — 15 + 6 + 1 tests (routing, write/read projection, trusted-FK override, FAIL-CLOSED drop, batch grouping, per-table error attribution, merge/sort/limit, read-error throw, resolver get-vs-create branch, end-to-end composition). Full stratum suite **289 passing + 5 todo** (was 267 at session start); typecheck clean.
