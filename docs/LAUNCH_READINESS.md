...
| v0.5.x | Phase 3 three-tier memory + knowledge-graph view | IN PROGRESS (Session 17: **Tier-2 warm-memory adapter BUILT** + live Supabase `initial_schema` applied to the Stratum project + **live SQL round-trip verified** + 15 unit tests + ADR-0012 + gated `verify-tier2` script. Tier-1 hot + Zod fact schemas + extractor built/tested in prior sessions. Tier-3 Neo4j + Pinecone still STUBS [gated on accounts]; knowledge-graph view + `/understand-codebase` not started) | ~30 | ~100 | ~23% | Fact-survives-50-turn; tier latencies met; `/understand-codebase` works |
...
- **Tested.** `test/memory/tier2.test.ts` — 15 tests (routing, write/read projection, trusted-FK override, FAIL-CLOSED drop, batch grouping, per-table error attribution, merge/sort/limit, read-error throw). Full stratum suite **282 passing + 5 todo** (was 267); typecheck clean.
