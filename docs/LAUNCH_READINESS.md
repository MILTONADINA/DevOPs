...
| v0.3.x | Phase 0 complete + Phase 1 measurement proxy + first-party Anthropic integrations | IN PROGRESS (§2a + §2f CLOSED + REDO-hardened Session 15; §2b/§2c/§2d/§2g/§2h pending) | ~13 | ~102 | ~11% | Friend can clone + setup + see live dashboard; `/security-review` GitHub Action gates PRs |
...
| **TOTAL to v1.0.0** | — | — | **168** | **~631** | **~21.0%** | — |

Math: 168h done ÷ ~799h total envelope = **~21.0% to v1.0.0** (headline **~21%**). Session 15 added ~13h to numerator (8h v0.3.x §2a code-side gaps closure + 3h §2f Anthropic integrations + ~2h §2a-redo adversarial re-verification & security remediation); envelope unchanged.
...
- **For "is the full vision done?"** → **~21%** (Option B locked, v0.3.x in progress). 155h (v0.2.0 SHIPPED) + 13h (Session 15: §2a code-side gaps + §2f Anthropic integrations + in-session validator-rot remediation + §2a-redo adversarial re-verification & security remediation) = **168h done / ~799h total envelope = 21.0% → headline ~21%**. First numerator-loaded implementation work since v0.2.0 sealing. v0.3.x ~102h remaining = §2b content corpus (~12h) + §2c Phase 0 closure (~few hours) + §2d Phase 1 measurement proxy (~80h) + §2g/§2h closure (~few hours) + release (~4h).
- **For "what's the next load-bearing decision?"** → v0.3.x §2b content corpus: 5+ real Claude Code session captures, fill `stratum/docs/waste-taxonomy.md` (load-bearing #1 waste finding drives Phase 2 pruner heuristics), DyCP paper notes. This is user-side activity (operator running the now-PII-safe capture proxy through real work). Engineering side (§2d Phase 1 measurement proxy buildout) blocks on §2b content for waste-detection heuristics calibration. PB-13 closure remains parallel-track via user-side spending-limit configuration (also blocks the new `CLAUDE_API_KEY`-dependent claude-security-review workflow from running successfully); PB-16 closure remains parallel-track dedicated session; PB-23 (scaffold typecheck cleanup) is gating precondition for §2d Phase 1 work.
