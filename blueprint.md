...
- **Phase 3 (Three-Tier Memory)** → **KEEP at full scope.** ADR-0013 approves Supabase pgvector and graph tables for v0.5; Pinecone and Neo4j remain optional future adapters if scale or traversal needs justify them.
...
│  - billing_records │         │     Supabase pgvector         │
...
| **Interactive knowledge-graph view of the codebase** | [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything) | Builds on the approved Supabase Tier-3 graph/vector stores (ADR-0013). The `understand-codebase` CLI exists; the `/dashboard/graph` view, universal slash command, fuzzy/semantic search, and guided tours remain planned. | Re-estimate open v0.5.x UI/integration work |
