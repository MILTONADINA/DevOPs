...
### AC-R10.1 (REQ-R10)
**Given** cycle 6's real `journal.jsonl` (run `wf_9f295fe7-e33`, tester T7 journaled, coder T8 journaled without its tester) and a `run.json` for it **When** `node scripts/graph-resume-args.mjs <cycleId>` runs **Then** the printed args contain the plan, `priorBuildResults` with T1–T7, `priorCoderResults` with exactly T8, and `resumedFrom: "wf_9f295fe7-e33"`.

### AC-R11.1 (REQ-R11)
**Given** `blocked.md` with class `needs_human` and a failing preflight **When** `/sprint --resume` is invoked **Then** nothing is launched and `blocked.md` is unchanged; **Given** the fault fixed and preflight `ready` **When** invoked again **Then** the Workflow launches with the derived args and `blocked.md` is removed.
